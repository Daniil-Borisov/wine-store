...
export const useGetOrderDetails = (order: any, params: any) => {
  if (!order) {
    return;
  }
  const { data, isLoading } = useQuery(
    [API_ENDPOINTS.GET_ORDER_DETAILS, { order: order.id, ...params }],
    ({ queryKey }) => client.orders.getOrderDetails(queryKey[1])
  );

  return { orderDetails: data?.data?.order, isLoading };
};
