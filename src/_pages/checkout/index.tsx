import { useTranslation } from 'next-i18next';
import dynamic from 'next/dynamic';
import { getLayout } from '@/components/layouts/layout';
import { AddressType } from '@/framework/utils/constants';
import Seo from '@/components/seo/seo';
import { useGetAddress, useUser, useUserCart } from '@/framework/user';
import { billingAddressAtom, shippingAddressAtom } from '@/store/checkout';
import StripeForm from '@/components/checkout/payment/stripe';
export { getStaticProps } from '@/framework/general.ssr';

const ScheduleGrid = dynamic(
  () => import('@/components/checkout/schedule/schedule-grid')
);
const AddressGrid = dynamic(
  () => import('@/components/checkout/address-grid'),
  { ssr: false }
);
const ContactGrid = dynamic(
  () => import('@/components/checkout/contact/contact-grid')
);
const RightSideView = dynamic(
  () => import('@/components/checkout/right-side-view'),
  { ssr: false }
);

export default function CheckoutPage() {
  const { t } = useTranslation();
  const { me } = useUser();
  const { cart } = useUserCart();
  const { id, address, profile } = me ?? {};
  const { addresses, isLoading } = useGetAddress();

  return (
    <>
      <Seo noindex={true} nofollow={true} />
      <div className="bg-gray-100 px-4 py-8 lg:py-10 lg:px-8 xl:py-14 xl:px-16 2xl:px-20">
        <div className="max-w-10xl m-auto flex w-full flex-col items-center rtl:space-x-reverse lg:flex-row lg:items-start lg:space-x-8">
          <div className="w-full space-y-6 lg:max-w-5xl">
            <AddressGrid
              userId={id!}
              className="bg-light p-5 shadow-700 md:p-8"
              label={t('text-billing-address')}
              //@ts-ignore
              addresses={addresses?.filter((item) => !item.wine_cellar)}
              atom={billingAddressAtom}
              // type={AddressType.Billing}
            />
            <AddressGrid
              userId={me?.id!}
              className="bg-light p-5 shadow-700 md:p-8"
              label={t('text-shipping-address')}
              //@ts-ignore
              addresses={addresses?.filter((item) => item.wine_cellar)}
              atom={shippingAddressAtom}
              // type={AddressType.Shipping}
            />
          </div>
          <div className="mt-10 mb-10 w-full sm:mb-12 lg:mb-0 lg:mt-0 lg:max-w-[598px]">
            <RightSideView cart={cart} />
            <StripeForm />
          </div>
        </div>
      </div>
    </>
  );
}
CheckoutPage.authenticationRequired = true;
CheckoutPage.getLayout = getLayout;
