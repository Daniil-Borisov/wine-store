...
export const userCardAtom = atom(
  (get) => get(checkoutAtom).card,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, card: data });
  }
);

export const threeDSAtom = atom(
  (get) => get(checkoutAtom).threeDS,
  (get, set, data: string) => {
    const prev = get(checkoutAtom);
    return set(checkoutAtom, { ...prev, threeDS: data });
  }
);