...
function ClearFiltersButton() {
  ...
  function clearFilters() {
    const { order, keyword, ...rest } = router.query;
    router.push({
...
const SidebarFilter: React.FC<{
  ...
  categories?: any;
  brands?: any;
}> = ({ type, showManufacturers = true, className, categories, brands }) => {
  const router = useRouter();
      ...
      <div className="flex-1 px-5">
        ...
        {categories ? (
          <FieldWrapper title="text-categories">
            <CategoryFilter type={type} categories={categories} />
          </FieldWrapper>
        ) : (
          ''
        )}
        ...
        {brands ? (
          <FieldWrapper title="text-brands">
            <CategoryFilter type={type} categories={brands} brands={true} />
          </FieldWrapper>
        ) : (
          ''
        )}
